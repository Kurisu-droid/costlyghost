...
export const buildBusinessQuotePdf = (quote: QuoteRecord, customer: Customer | undefined, lines: QuoteLineRecord[]): Buffer => {
  ...
  if (quote.paymentTerms) text.push(`BT /F1 9 Tf 50 58 Td (Payment Terms: ${esc(quote.paymentTerms)}) Tj ET`);
  if (quote.validityDate) text.push(`BT /F1 9 Tf 50 44 Td (Validity: ${esc(quote.validityDate)}) Tj ET`);
