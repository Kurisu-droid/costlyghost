export class ImportService {
    ...
    mappingTemplates;
    expenseValues;
    constructor(batches, rows, candidates, proposedPrices, proposedExpenses, mappingTemplates, expenseValues) {
        this.batches = batches;
        this.rows = rows;
        this.candidates = candidates;
        this.proposedPrices = proposedPrices;
        this.proposedExpenses = proposedExpenses;
        this.mappingTemplates = mappingTemplates;
        this.expenseValues = expenseValues;
    }
    createBatch(fileName, sourceFormat, rawRows) {
        const batch = {
    ...
    saveTemplate(templateName, sourceFormat, mapping) {
    ...
    listTemplates(sourceFormat) {
        return sourceFormat ? this.mappingTemplates.filter((t) => t.sourceFormat === sourceFormat) : this.mappingTemplates;
    }
    generateProposals(batchId, parsedRows, suppliers, prices, expenseDefinitions, periodYear) {
        ...
        this.proposedPrices.splice(0, this.proposedPrices.length, ...this.proposedPrices.filter((c) => c.batchId !== batchId));
    ...
    remapRow(rowId, type, candidateRefId) {
        if (type === 'expense') {
            const pe = this.proposedExpenses.find((p) => p.rowId === rowId);
            if (pe)
                pe.expenseDefinitionId = candidateRefId;
            return;
        }
    ...
    splitRow(rowId, splitAmounts) {
        const row = this.rows.find((r) => r.id === rowId);
        if (!row || splitAmounts.length < 2)
            return [];
    ...
    approveSelected(batchId, approvedPriceRowNumbers, approvedExpenseIds) {
    ...
    matchPriceCandidates(row, prices, supplierId) {
        ...
        if (byCode)
            return [{ candidateType: byCode.itemType, candidateRefId: byCode.itemId, confidence: 1, status: 'exact', unitMismatch: false, supplierAwareRank: byCode.supplierId === supplierId ? 1 : 2 }];
...
const normalize = (input) => input.toLowerCase().replace(/[.,/\\-]/g, ' ').replace(/\s+/g, ' ').trim();
const statusFromScore = (score) => (score >= 0.92 ? 'exact' : score >= 0.6 ? 'possible' : 'none');
...
const netAmount = (row) => {
    ...
    if (desc.includes('vat') || desc.includes('φπα') || desc.includes('tax'))
        return 0;
    return gross;
};
