export class ExpenseService {
    ...
    hourlySnapshots;
    finalizedYears;
    constructor(definitions, values, parameterVersions, importMatches, allocationRules, hourlySnapshots, finalizedYears) {
        this.definitions = definitions;
        this.values = values;
        this.parameterVersions = parameterVersions;
        this.importMatches = importMatches;
        this.allocationRules = allocationRules;
        this.hourlySnapshots = hourlySnapshots;
        this.finalizedYears = finalizedYears;
    }
    createManualValue(input) {
        ...
        if (!definition)
            throw new Error('Expense definition not found');
        if (input.amount < 0)
    ...
    finalizeYear(year) {
        if (this.finalizedYears.has(year))
            throw new Error('Year already finalized');
        const summary = this.summaryByYear(year);
        const parameter = this.latestParameterForYear(year);
        if (!parameter)
            throw new Error('No production parameters available for this year');
        ...
        if (parameter.utilizationPercent <= 0)
            throw new Error('Utilization must be greater than zero');
        if (netHoursPerYear <= 0)
            throw new Error('Zero net production hours: finalization blocked');
        const snapshot = {
    ...
    latestParameterForYear(year) {
        return [...this.parameterVersions]
            .filter((entry) => new Date(entry.effectiveFrom).getFullYear() <= year)
            .sort((a, b) => b.effectiveFrom.localeCompare(a.effectiveFrom))[0];
    }
    validateParameterInput(utilizationPercent, activeMachines, workingDaysPerYear, hoursPerDay) {
        if (!utilizationPercent || utilizationPercent <= 0)
            throw new Error('Utilization must be greater than zero');
        if (utilizationPercent > 1)
            throw new Error('Utilization must be expressed as 0..1');
        if (activeMachines <= 0 || workingDaysPerYear <= 0 || hoursPerDay <= 0)
            throw new Error('Production parameters must be positive values');
    }
    assertYearUnlocked(year) {
        if (this.finalizedYears.has(year))
            throw new Error('Year is finalized and locked');
    }
}
const sectionTotal = (sections, section) => sections.find((entry) => entry.section === section)?.total ?? 0;
