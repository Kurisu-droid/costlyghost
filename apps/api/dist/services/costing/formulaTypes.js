const overheadPerHourFromSet = (set) => {
...
class BaseFormula {
    ...
    validate(product) {
        const errors = [];
        if (product.wasteValue < 0)
            errors.push('Waste value cannot be negative.');
        if (!['percentage', 'fixed'].includes(product.wasteMode))
            errors.push('Waste mode must be percentage or fixed.');
        return errors;
    }
    baseWaste(product, baseCost) {
        return product.wasteMode === 'percentage' ? (baseCost * product.wasteValue) / 100 : product.wasteValue;
    }
    overheadPerPiece(product, overheadSet, fallbackRate = 1) {
...
class WorkbookPendingFormulaBase extends BaseFormula {
    pendingResult(context, typeName) {
...
export class Type2StretchFormulaService extends WorkbookPendingFormulaBase {
    type = 2;
    calculate(context) { return this.pendingResult(context, 'type2'); }
}
export class Type3BakingPaperFormulaService extends WorkbookPendingFormulaBase {
    type = 3;
    calculate(context) { return this.pendingResult(context, 'type3'); }
}
export class Type4NapkinFormulaService extends WorkbookPendingFormulaBase {
    type = 4;
    calculate(context) { return this.pendingResult(context, 'type4'); }
}
export class Type5TissueFormulaService extends WorkbookPendingFormulaBase {
    type = 5;
    calculate(context) { return this.pendingResult(context, 'type5'); }
}
export class Type6TradeGoodsFormulaService extends WorkbookPendingFormulaBase {
    type = 6;
    calculate(context) { return this.pendingResult(context, 'type6'); }
}
