...
export const QuotesPage = () => {
  ...
  const addSuggestion = (p: ProductSuggestion) => {
    setLines((prev) => [...prev, { sku: p.sku, description: p.description, subcategory: p.subcategory, quantity: 1, tier: 1, unitPrice: p.prices.tier1, discountPercent: 0, discountAmount: 0, lineSubtotal: p.prices.tier1, netLineTotal: p.prices.tier1 }]);
    setQuery('');
    setSuggestions([]);
  };
  ...
  const updateLine = (idx: number, patch: Partial<Line>) => setLines((prev) => prev.map((line, i) => (i === idx ? recalc({ ...line, ...patch }) : line)));
  ...
  const saveDraft = async () => {
    const result = await apiPost<any>('/quotes', {
      ...
      lines: lines.map((l) => ({ sku: l.sku, quantity: l.quantity, tier: l.tier, manualUnitPrice: l.unitPrice, discountPercent: l.discountPercent, discountAmount: l.discountAmount }))
    });
  ...
  return (
    <section className="split quote-layout">
      <div>
        <header className="page-head"><h2>Quotes</h2><p>Fast keyboard-friendly quote builder.</p></header>
        <div className="panel">
          <input autoFocus placeholder="Type SKU or description..." value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && suggestions[0]) addSuggestion(suggestions[0]); }} />
          {suggestions.length > 0 && <ul className="suggestions">{suggestions.map((s) => <li key={s.sku}><button onClick={() => addSuggestion(s)}>{s.sku} — {s.description} · {s.subcategory}</button></li>)}</ul>}
        </div>
        <div className="panel">
          <table className="dense"><thead><tr><th>SKU</th><th>Description</th><th>Subcategory</th><th>Qty</th><th>Tier</th><th>Unit</th><th>Disc%</th><th>Disc€</th><th>Net</th></tr></thead><tbody>
            {lines.map((line, idx) => <tr key={`${line.sku}-${idx}`}>
              <td>{line.sku}</td><td>{line.description}</td><td>{line.subcategory}</td>
              <td><input type="number" value={line.quantity} onChange={(e) => updateLine(idx, { quantity: Number(e.target.value) })} /></td>
              <td><select value={line.tier} onChange={(e) => updateLine(idx, { tier: Number(e.target.value) as 1 | 2 | 3 | 4 })}><option value={1}>T1</option><option value={2}>T2</option><option value={3}>T3</option><option value={4}>T4</option></select></td>
      ...
      <aside className="panel sticky-summary">
        ...
        <p><strong>Total: {totals.net.toFixed(2)} EUR</strong></p>
